...
class FeatureExtractor:
    ...
    # paper 에서 V. LIDAR ODOMETRY 부분에서 four identical subregions로 나누다 함.
    # 5 points needed to make 4 regions.
    FEATURES_REGION = 5


    def get_curvatures(self, pcd):
        filter_for_convolve = [1, 1, 1, 1, 1, -10, 1, 1, 1, 1, 1]   # paper V. LIDAR ODOMETRY equation (1) => SUM(X_(k,i) - X_(k,j))
        assert len(filter_for_convolve) == 2 * self.FEATURES_REGION + 1
        operated_convolve_value = lambda x: np.convolve(x, filter_for_convolve, 'valid')
        x_diff = operated_convolve_value(pcd[:, 0]) # point cloud에서 x 좌표에서의 차이의 합 (= x 좌표들에 대한 conv 연산)
        y_diff = operated_convolve_value(pcd[:, 1]) # '' y 좌표에서의 차이의 합
        z_diff = operated_convolve_value(pcd[:, 2])
        curvatures = x_diff * x_diff + y_diff * y_diff + z_diff * z_diff # => ||SUM (X_(k,i) - X_(k,j))||
        curvatures /= np.linalg.norm(pcd[self.FEATURES_REGION : -self.FEATURES_REGION], axis=1) * 10 # S = 10
